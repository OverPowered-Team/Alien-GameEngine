...
void ModuleCamera3D::CreateRay()
{
	...
	for (; it != hits_aabb.end(); ++it) {
		...
		if (TestTrianglesIntersections((*it).second, ray, distance))
			hits_triangle.push_back({ distance ,(*it).second });
	}
	...
	if (!hits_triangle.empty()) {
		selected_go = hits_triangle[0].second;
...
bool ModuleCamera3D::TestTrianglesIntersections(GameObject* object, const LineSegment& ray, float& min_dist)
{
	bool ret = false;
